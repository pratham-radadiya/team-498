...
| **Per-request user validation** | **Every API route (except `/api/auth/*`) re-validates the requesting user before running any business logic** | Not just role-gating a few route groups — a shared `withAuth()` wrapper runs on every single route handler, decodes the session, then re-checks that `Employee.status === 'Active'` in the database on that request (never trusts the JWT payload alone, since a deactivated/deleted account's existing token would otherwise keep working until it expires). ⚠️ **Updated post-Phase 6, by explicit decision:** originally this checked a separate `User.status`; `User` was later merged into `Employee` — see the architecture-change note after Phase 6 below. |
...
**Rule enforced on every route without exception (except `/api/auth/*`):** `route.js` calls `withAuth(request)` as its first line. `withAuth()` decodes the NextAuth session, then queries `Employee` by id and confirms `status === 'Active'` — a request with a technically-valid but stale JWT (account deactivated by Admin after the token was issued) is rejected with 401 on that very request, not just at next login. `withAuth()` returns `{ employeeId, role }` for the controller to use (no separate `userId` — `employeeId` **is** the identity, since `Employee` is the login account; see the architecture-change note after Phase 6); role-specific routes additionally call `requireRole(session, [...allowedRoles])` from the same file. `proxy.js` (Next.js 16's renamed `middleware.js`, `export function proxy(request)`) is a fast first-pass reject at the edge (no valid session cookie at all → redirect/401 immediately) but is **not** relied on as the sole guard — `withAuth()` inside the route is the authoritative check, since proxy/middleware can be bypassed by direct route invocation in tests or misconfigured matchers.
...
|---|---|---|---|---|---|
| Employees (also the login/account for each — see architecture-change note after Phase 6) | Read (own record only) | Read/Update/Delete | Read/Update/Delete | Read/Update/Delete | Full CRUD — **Create is Admin-only**, since creating an Employee also provisions its password + role |
...
- No role can grant or elevate its own `role` field — enforced in `employee.service.js`'s `updateEmployee()`: changing `role` on your own record is always `403`, and changing anyone else's `role` requires Admin. (Originally this lived in a separate User-management flow; folded into Employee updates after the merge — see the architecture-change note after Phase 6.)
...
- [x] `User` model: `id, email, passwordHash, role (enum), employeeId (nullable, becomes a real FK once Employee exists in Phase 1), status (Active/Inactive)` — ⚠️ **this model no longer exists**, see the architecture-change note after Phase 6: it was merged directly into `Employee` (which now carries `passwordHash` + `role` itself), by explicit decision after Phase 6 shipped. Left here as a historical record of what Phase 0 actually built at the time.
...
- [x] `rbac/guards.js` — `withAuth(request)` (decode session, re-check `Employee.status === 'Active'` in the DB, return `{ employeeId, role }` or throw 401) + `requireRole(session, allowedRoles)` (throw 403). **`withAuth()` is written once here and imported into literally every route file from Phase 1 onward.** (Originally checked a separate `User` table and returned `{ userId, employeeId, role }` — updated post-Phase 6, see the architecture-change note after Phase 6.)
...
- [x] NextAuth config (`lib/auth.js`): Credentials provider (bcrypt compare), JWT callback embeds `{ role, employeeId }` (originally `{ userId, role, employeeId }` against a separate `User` table — updated post-Phase 6, see the architecture-change note after Phase 6)
...
- **Verify — all confirmed with real evidence, `next dev` running against the local DB:**
  - `resolveActiveUser()` (renamed `resolveActiveEmployee()` post-Phase 6, see the architecture-change note after Phase 6) tested directly against the DB: `null` id → rejected; seeded Admin → resolved `{role: "ADMIN", ...}`; Admin's `status` flipped to `Inactive` in the DB → immediately rejected on next check (proves the DB re-check, not just JWT trust); reactivated afterward
...
> ⚠️ **This section describes Phase 1 as originally built** (separate `User`/`Employee` models). **That has since changed by explicit decision** — `User` was merged into `Employee` after Phase 6 shipped; see the architecture-change note after Phase 6 for the full rationale and what actually changed. The bullets below are left as a historical record of Phase 1's original shape; `Docs/api/phase-1-employee-user.md` reflects the **current** (merged) contract — read that, not this, for what to build against today.

- **Models (original):** `Employee` (department, manager self-FK, workLocation, jobPosition, workingScheduleId, status, company), `User` link — **now merged: `Employee` itself carries `passwordHash` + `role`, there is no `User` model.**
- **Files (original):** `employee.validator.js`, `employee.controller.js`, `employee.service.js`, `employee.repository.js` (+ same 4 for `user`) — **the 4 `user.*` files were deleted in the merge.**
- **Routes (original):** `POST /api/employees` (create), `POST /api/employees/list` (AG Grid), `GET /api/employees/options`, `GET/PATCH/DELETE /api/employees/[id]`; `POST/GET /api/users` (Admin only) — **`/api/users/*` no longer exists.** `POST /api/employees` is now Admin-only (it provisions the login too) — every route still calls `withAuth()` first.
- **Business rules (original, now superseded — current rules in the architecture-change note):**
  - Employee is the central record other modules link to — still true, more so now (it's also the account)
  - A User must link to an Employee; role is assigned only at creation by Admin → **now:** an Employee's own `role` is set directly at creation, Admin-only
  - A user can never change their own `role` field (guard on self-PATCH, enforced in `user.service.js`) → **now:** same rule, enforced in `employee.service.js`'s `updateEmployee()` instead
  - Per the permission matrix: EMPLOYEE role reads only their own record; all other roles get full CRUD (User Management stays Admin-only) → **now:** read/update/delete stay open to the wider non-Employee role set, only **create** narrowed to Admin
  - ⚠️ Deferred: smart-button counts (contracts/attendance/timeoff) can't be implemented yet — those models don't exist until Phases 2–4. Revisit once they land. *(Resolved in Phase 4 — see "Pending items from earlier phases, completed now" below.)*
- **Hook:** `useEmployeesGrid.js`
- **Agent:** `rain-skill:backend-specialist`
- **Verify — as originally run against the (now-superseded) separate User/Employee models:**
  - Admin created an Employee (201), listed via grid (`rows`/`rowCount` correct), fetched by id, updated (PATCH), deleted (204) — still holds true today, this part didn't change
  - Admin created a User linked to that Employee with role `EMPLOYEE`; duplicate user for the same `employeeId` → **409**; missing required fields on create → **400** with Zod issue details — ⚠️ superseded: re-verified post-merge as "duplicate email on Employee create → 409", see the architecture-change note
  - Logged in as the EMPLOYEE user: reading own record → **200**; reading a different employee's record → **403** "You may only view your own employee record"; grid list forced to own record only regardless of filters; attempting to `POST /api/employees` (create) → **403** "Insufficient role for this action" — still holds true today (Employee role was never allowed to create, before or after the merge)
  - Admin attempting to `PATCH` **their own** role → **403** "You cannot change your own role"; Admin changing a *different* user's role → succeeds — still holds true today, same rule, different file
...
- **Verify — all confirmed against the running server + real DB:**
  ...
  - Admin corrected a record's `checkIn`/`checkOut` to a synthetic 10-hour Monday shift → `workedHours: 10`, `overtime: 2.5` against the employee's 7.5h Monday schedule entry, `correctedBy` set to the Admin's id (originally the Admin's `userId`; now the Admin's own `employeeId`, since Employee is the login account — see the architecture-change note after Phase 6)
...
`prisma/seed.js` (extended, not replaced) now seeds a full deterministic dataset (`faker.seed(12345)`, wipe-and-reseed, guarded against `NODE_ENV=production` and non-local `DATABASE_URL`): 6 named anchor Employees with real login credentials (one per role, fixed demo passwords, plus one deliberately `Inactive`), 21 Employees total, 4 Working Schedules, 24 Contracts (some with Expired+Running history), 252 Attendance rows (30 weekdays × 12 employees), 3 Salary Structures with all 19 Rules (the PDF's exact 12-rule "Regular Salary" chain plus lighter Intern/Contractor structures), 3 Time Off Types, 12 Allocations, 9 Time Off Requests (6 Approved, 2 Pending, 1 Refused). Verified idempotent — re-running produced identical counts. ⚠️ **Re-seeded post-merge** (see the architecture-change note after Phase 6): the 6 anchors were originally separate Employee + User rows (the User rows deleted on every reseed); now each is a single Employee row carrying its own `role`/`passwordHash` directly, including a `System Admin` employee — the old Admin had no linked Employee at all, which is no longer possible once Employee *is* the account.
...
| `g00998650+inactive@gmail.com` | `Employee@123` | EMPLOYEE (Karan Singh), status `Inactive` — for testing the deactivated-login rejection. ⚠️ Originally this was a `User.status: Inactive` on an otherwise-`Active` linked Employee ("a real person whose login got disabled, not an inactive employee"); post-merge there's only one `status` field, so this row is now simply an Inactive Employee — the "disabled login, active person" nuance from the original design no longer has a way to be expressed |

The `g00998650+admin@gmail.com` login above didn't exist as a named row before the merge — the original Admin `User` had `employeeId: null` (no linked Employee at all). Post-merge, every login must be an Employee, so seeding now creates a `System Admin` Employee to hold it.
...
### Post-Phase 6 architecture change: `User` merged into `Employee`, on request

After Phase 6 shipped, the team decided to remove the separate `User` model entirely and fold its two fields (`passwordHash`, `role`) directly onto `Employee` — **Employee is now the login account.** This reverses the original Phase 0/1 design (a `User` linked to an `Employee` via a nullable `employeeId` FK), which itself matched the Excalidraw mockup's own stated intent ("User accounts are separate from Employee records, but should be linked to an employee for access and ownership" — `Docs/HRMS OXP - 24 hours.excalidraw`, "0) Login & User Access Flow"). That spec text was surfaced and explicitly overridden by the team's own call, not missed or ignored.

**Why:** the practical trigger was a frontend bug report — creating a new Employee via Admin, with a role assigned, showed as "unassigned" in the Employee list, because the two records were separate and the list query wasn't joining them. Tracing that led to the harder question of whether keeping them separate was worth the complexity at all, and the team decided it wasn't for this project's scope: every login in this system corresponds to exactly one person's HR record, there's no real case here of a login existing without one (or vice versa), so the extra model was pure indirection.

**What actually changed:**
- `schema.prisma`: `User` model deleted entirely (and the now-unused `UserStatus` enum, since `Employee` already had an equivalent `EmployeeStatus`). `Employee` gained `passwordHash String` and `role Role`, both required.
- **Migration mechanics, non-obvious enough to record:** `prisma migrate dev` refuses to run non-interactively when adding required columns to non-empty tables (needs an interactive default-value prompt) — worked around by hand-writing the migration SQL (temporary `DEFAULT ''`/`DEFAULT 'EMPLOYEE'` on the two new columns, immediately dropped again after backfill, since the very next step reseeds every row anyway) and applying it with `prisma migrate deploy` (which runs non-interactively) instead of `migrate dev`.
- `server/lib/auth.js`: `authorize()` now looks up `Employee` by email directly and compares its `passwordHash`. Session/JWT shape simplified from `{ userId, role, employeeId }` to `{ role, employeeId }` — no separate identity field, since `employeeId` is now the whole identity.
- `server/rbac/guards.js`: `withAuth()`/`resolveActiveUser()` (renamed `resolveActiveEmployee()`) now re-check `Employee.status` instead of `User.status`.
- **Security-sensitive follow-through, easy to miss:** every place that used to `include`/return a full `Employee` relation now goes through an explicit `EMPLOYEE_SAFE_SELECT` (every real field except `passwordHash`) in `employee.repository.js`, re-used from `payslip.repository.js` and `payrun.repository.js` wherever they touch Employee data — Employee carrying a secret now means every read of it needs to say so explicitly, it can't just be trusted to fall out of "no `select` = return everything."
- Deleted entirely: `server/{controllers,services,repositories,validators}/user.*.js`, `app/api/users/`.
- `employee.validator.js`'s `createEmployeeSchema` gained required `password`/`role`; `updateEmployeeSchema` explicitly excludes `password` (no password-change flow exists) and keeps `role` but blocks it in the service layer for the same two cases the old `user.service.js` blocked: changing your own role, or a non-Admin changing anyone's role.
- `POST /api/employees` narrowed from the wider non-Employee role set to **Admin-only**, since creating an Employee now also provisions a login — matching the old Users-module create restriction.
- `prisma/seed.js`: the 6 named anchors (one per role, one deliberately Inactive) are now single Employee rows with their own `passwordHash`/`role`, including a new `System Admin` Employee — the old Admin `User` had no linked Employee at all, which the merged model can't represent (every login must be *some* Employee now).
- Every phase's API contract doc under `Docs/api/` updated to match (`phase-1-employee-user.md` most heavily, since it owned the old User endpoints; `phase-3-attendance.md`'s `correctedBy` wording).

**What this trades away**, worth being honest about rather than silently accepting: the mockup's own distinction — a person can exist in HR (Employee) before or without ever getting a login (User) — is gone. Every Employee now needs a password at creation, whether or not that person will ever actually log in. For this project's scope (a hackathon build where every seeded/demo Employee logs in) that trade was judged worth the simplicity; a real production HRIS with contractors/pre-onboarding hires who shouldn't have credentials yet would likely want the original separation back.
...
- [ ] Seed at least one Employee login per role (EMPLOYEE, HR_MANAGER, HR_PAYROLL_USER, HR_PAYROLL_MANAGER, ADMIN), plus one Inactive one, so Phase 8's per-module role checks and the deactivated-login check are actually runnable end-to-end (✅ already done — see the seed-data section above)
