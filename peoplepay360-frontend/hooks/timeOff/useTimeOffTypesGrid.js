...
import { listTimeOffTypes } from "@/lib/api/timeOffTypeApi";

const DEFAULT_PAGE_SIZE = 20;

/**
 * Drives the Time Off Types List against POST /api/timeoff/types/list.
 * Filterable columns: name (contains), status (equals) — per
 * Docs/api/phase-4-time-off.md.
 */
export default function useTimeOffTypesGrid({ pageSize = DEFAULT_PAGE_SIZE } = {}) {
  ...
  const fetchPage = useCallback(async () => {
    ...
    try {
      ...
      const data = await listTimeOffTypes({ startRow, endRow, sortModel, filterModel });
      setRows(data.rows ?? []);
