...
import AllocationTable from "@/components/time-off/AllocationTable";
...
export default function AllocationsPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        title="Allocations"
        description="Leave balances — Allocated, Taken and Remaining at a glance."
        actions={
          can("allocations", "create") && (
            <Link
              href={employeeId ? `/time-off/allocations/new?employeeId=${employeeId}` : "/time-off/allocations/new"}
      ...
      {employeeId && (
        <div className="flex items-center justify-between rounded-md bg-indigo-50 px-3 py-2 text-sm text-indigo-800 dark:bg-indigo-500/10 dark:text-indigo-300">
          <span>Showing allocations for {employeeName || "this employee"}</span>
          <Link href="/time-off/allocations" className="flex items-center gap-1 font-medium hover:underline">
      ...
      <AllocationTable employeeId={employeeId} />
    </div>
  );
}
